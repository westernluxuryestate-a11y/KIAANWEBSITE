...
import { PropertyContextCalculatorModal } from './PropertyContextCalculatorModal';
...
export const AssetExperienceView: React.FC<AssetExperienceViewProps> = ({
  ...
  const [showContextCalculator, setShowContextCalculator] = useState(false);
  const [calculatorInitialTab, setCalculatorInitialTab] = useState<'AI_QUERY' | 'AFFORDABILITY' | 'INVESTMENT' | 'BUY_VS_RENT' | 'EMI' | 'SENSITIVITY'>('AI_QUERY');
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            >
              ...
              <div className="space-y-3 pt-2">
                ...
                {/* Contextual Kiaan AI & Financial Engines Trigger */}
                <div className={`p-4 rounded-2xl border space-y-3 ${isDark ? 'bg-amber-500/10 border-amber-500/25' : 'bg-amber-50/70 border-amber-200'}`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold text-amber-600 dark:text-amber-400 flex items-center gap-1.5 uppercase tracking-wide">
                      ...
                      <span>Contextual Intelligence Engine</span>
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => {
                        setCalculatorInitialTab('AFFORDABILITY');
                        setShowContextCalculator(true);
                      }}
                      className="px-2.5 py-2 rounded-xl text-left bg-black/20 hover:bg-black/30 dark:bg-white/5 dark:hover:bg-white/10 text-xs font-semibold text-current border border-current/10 transition-all flex items-center justify-between"
                    >
                    ...
                    <button
                      onClick={() => {
                        setCalculatorInitialTab('INVESTMENT');
                        setShowContextCalculator(true);
                      }}
                      className="px-2.5 py-2 rounded-xl text-left bg-black/20 hover:bg-black/30 dark:bg-white/5 dark:hover:bg-white/10 text-xs font-semibold text-current border border-current/10 transition-all flex items-center justify-between"
                    >
                    ...
                    <button
                      onClick={() => {
                        setCalculatorInitialTab('BUY_VS_RENT');
                        setShowContextCalculator(true);
                      }}
                      className="px-2.5 py-2 rounded-xl text-left bg-black/20 hover:bg-black/30 dark:bg-white/5 dark:hover:bg-white/10 text-xs font-semibold text-current border border-current/10 transition-all flex items-center justify-between"
                    >
                      <span>⚖️ Buy vs Rent</span>
                      <ArrowRight className="w-3 h-3 text-amber-500" />
                    </button>
                    <button
                      onClick={() => {
                        setCalculatorInitialTab('AI_QUERY');
                        setShowContextCalculator(true);
                      }}
                      className="px-2.5 py-2 rounded-xl text-left bg-amber-500/20 hover:bg-amber-500/30 text-xs font-bold text-amber-600 dark:text-amber-300 border border-amber-500/30 transition-all flex items-center justify-between"
                    >
        ...
        {/* ========================================================================= */}
        {/* SECTION 4.5: CONSTRUCTION ROADMAP & VERIFIED MILESTONES                   */}
        {/* ========================================================================= */}
        <section id="construction-roadmap" className="pt-2">
      ...
      {showContextCalculator && (
        <PropertyContextCalculatorModal
          isOpen={showContextCalculator}
          onClose={() => setShowContextCalculator(false)}
          initialTab={calculatorInitialTab}
          property={{
            id: selectedUnit.id || project.id,
            title: `${project.name} ${selectedUnit.unitNumber ? `· Unit ${selectedUnit.unitNumber}` : ''}`,
            price: unitPrice,
            carpetAreaSqFt: selectedUnit.carpetAreaSqFt || project.carpetAreaRangeSqFt.min || 1250,
            configuration: selectedUnit.configuration || project.configurations[0] || '3 BHK',
            microMarket: project.location.microMarket || 'Pune West',
            city: project.location.city || 'Pune',
            projectStatus: project.constructionStatus || 'Under Construction',
            estimatedMonthlyRent: Math.round(unitPrice * 0.0035), // ~4.2% gross yield estimate
          }}
        />
      )}
    </div>
